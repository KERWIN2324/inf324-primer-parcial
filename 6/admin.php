...
<a href="tipo_impuesto.php"><button class='btn btn-outline-dark'>VER POR TIPO DE IMPUESTO</button></a>
...
<table class="table table-bordered" style="width: 1200px; margin-left: auto; margin-right: auto; border: 2px; text-align: center; vertical-align: middle;">
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td> 
            <form action='http://localhost:8090/tipo_impuesto/recibe' method='GET'>
                    <input type='hidden' name='id' value='" . $row['id'] . "'>
                    <button type='submit' class='btn btn-outline-dark'>Impuesto</button>
            </form></td>";
        echo "</tr>";
    }
    ?>
